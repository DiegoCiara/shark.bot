...
import { CheckCircle2, ChevronsUpDown, Contact2, Phone, Search } from 'lucide-react';
      ...
      <PopoverTrigger asChild>
        ...
        >
          {value && selectedOption
            ? `${selectedOption.name} - ${formatCpfCnpj(
                selectedOption.cpf_cnpj,
              )}`
            : placeholder}
        ...
        <div className="flex flex-col gap-2 p-2 realative">
          ...
          <div className="max-h-60 overflow-y-auto p-1">
            ...
            {filteredOptions.map((framework) => (
              <Button
                key={framework.cpf_cnpj}
                className={`flex items-center relative h-auto w-full justify-start text-left p-2 ${value === framework.id && 'border border-gray-200 '}`}
                variant="ghost"
                onClick={() => {
              ...
              >
                ...
                <div className="flex flex-col items-start gap-2">
                  ...
                  <span className="text-xs">
                    {formatCpfCnpj(framework.cpf_cnpj)}
                  </span>
                </div>
